...
#define _INPUT_
      ...
      while ( set_distributions < num_distributions ) {
        ...
        for ( auto d : input_distributions ) {
          ...
          if ( data == "double" ) {
            ...
            else if (type == "linear_aniso2"){
                Dist = std::make_shared< linear_aniso_distribution2 > (name);
            }
            else if (type == "eDependentXS"){
                double a  = d.attribute("a").as_double();
                double b  = d.attribute("b").as_double();
                Dist = std::make_shared< eDependentXS_distribution > (name, a, b);
            }
            else {
          ...
          // integer-valued distributions
            ...
            if ( type == "delta" ) {
            ...
            else if ( type == "forward_peaked" ) {
              Dist = std::make_shared< forwardPeaked_distribution > ( name );
            }
            else if ( type == "shell" ) {
             ...
             Dist = std::make_shared< shell_distribution > ( name, i, o );
              ...
              Dist = std::make_shared< xbeam_distribution > ( name, x, y, z, rad );
            }
      ...
      for ( auto n : input_nuclides ) {
        ...
        double      a_num = n.attribute("atomic_weight").as_double();
      
        std::shared_ptr< nuclide > Nuc = std::make_shared< nuclide > ( n.attribute("name").value(), a_num );
        ...
        for ( auto r : n.children() ) {
          ...
          if ( rxn_type == "capture" ) {
            std::string E_dist_name = r.attribute("Edistribution").value();
            std::shared_ptr< distribution<double> > E_dist = findByName( double_distributions, E_dist_name );
            if ( E_dist ) {
              Nuc->addReaction( std::make_shared< capture_reaction > ( E_dist ) );
            }
            else {
              std::cout << " unknown energy dependent distribution " << E_dist_name << " in nuclide " << name << std::endl;
              throw;
            }
          }
            ...
            std::string E_dist_name = r.attribute("Edistribution").value();
            std::shared_ptr< distribution<double> > E_dist = findByName( double_distributions, E_dist_name );
            if ( scatterDist && E_dist ) {
              Nuc->addReaction( std::make_shared< scatter_reaction > ( E_dist, scatterDist ) );
            }
            else {
              std::cout << " unknown scattering or energy dependent distribution " << dist_name << " in nuclide " << name << std::endl;
              throw;
            }
          }
            ...
            std::shared_ptr< distribution<double> > E_dist = findByName( double_distributions, E_dist_name );
            if ( multDist ) {
              Nuc->addReaction( std::make_shared< fission_reaction > ( E_dist, multDist ) );
            }
            else {
              std::cout << " unknown multiplicity or energy dependent distribution " << mult_dist_name << " in nuclide " << name << std::endl;
              throw;
            }
          }
          else {
      ...
      for ( auto s : input_surfaces ) {
        ...
        if ( type == "plane" ) {
          ...
          double      c    = s.attribute("c").as_double();
          double      d    = s.attribute("d").as_double();
          S = std::make_shared< cone_x > ( name, a, b, c, d );
        }
        else {
      ...
      for ( auto e : input_estimators ) {
        ...
        if ( type == "current" ) {
          ...
          Est = std::make_shared< cell_pathLengthFluxCapture_estimator > ( name );
        ...
        else if ( type == "time_binned_scatter_pathLength_flux" ) {
          Est = std::make_shared< cell_pathLengthFluxScatter_estimator > ( name );
