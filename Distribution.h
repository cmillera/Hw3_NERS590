...
class distribution {
  ...
  public:
    ...
    virtual T sample2(particle* p) = 0;  // dummy function that must be implemented in each case
};
...
class arbitraryDelta_distribution : public distribution<T> {
  ...
  public:
    ...
    T sample2(particle* p) { return result; }
};
...
class linear_aniso_distribution2 : public distribution<double> {
  private:
  public:
     linear_aniso_distribution2( std::string label ) : distribution(label) {};
    ~linear_aniso_distribution2() {};
    double sample();
   double sample2(particle* p){};
};
...
class isotropicDirection_distribution : public distribution<point> {
  ...
  public:
    ...
    point sample();
    point sample2(particle* p){};
};

class forwardPeaked_distribution : public distribution<point> {
  private:
    const double twopi = 2.0 * std::acos(-1.0);
  public:
     forwardPeaked_distribution( std::string label ) : distribution(label) {};
    ~forwardPeaked_distribution() {};
    point sample();
    point sample2(particle* p){};
};
...
class xbeam_distribution : public distribution<point>{
  private:
    const double twopi = 2.0 * std::acos(-1.0);
    double x0,y0,z0,Rad;
  public:
     xbeam_distribution( std::string label, double p1, double p2, double p3, double p4 ) 
        : distribution(label), x0(p1), y0(p2), z0(p3), Rad(p4) {};
    ~xbeam_distribution() {};
    point sample();
    point sample2(particle* p){};
...
class eDependentXS_distribution : public distribution<double>{
private:
    double a,b;
    
  public:
     eDependentXS_distribution( std::string label, double p1, double p2) 
        : distribution(label), a(p1), b(p2) {};
    ~eDependentXS_distribution(){};
    double sample();
    double sample2(particle* p);
};
