...
double cylinder::distance( ray r ) {
  point p = r.pos;
  point u = r.dir;
...
point cylinder::reflect( ray r ) {
   ...
   point q( p.y - y0, p.z - z0 );

   double t = 2.0 * ( q.x * u.x  +  q.y * u.y  +  q.z * u.z ) / ( rad*rad );
   return point( u.x - q.x * t,  u.y - q.y * t,  u.z - q.z * t );
}

double cone_x::eval( point p ) {
  return std::pow( p.y - y0, 2 ) + std::pow( p.z - z0, 2 )  - std::pow( p.x - x0, 2 )*rad*rad;
}

double cone_x::distance( ray r ) {
  point p = r.pos;
  point u = r.dir;
  ...
  // put into quadratic equation form: a*s^2 + b*s + c = 0, where a = 1
  double a = u.y*u.y + u.z*u.z - rad*rad*u.x*u.x;
  double b = 2.0 * ( q.y * u.y  +  q.z * u.z  -  q.x * u.x * rad * rad);
  double c = eval( p );

  return quad_solve( a, b, c );
}

point cone_x::reflect( ray r ) {
   //probably wrong
   assert( std::fabs( eval( r.pos ) ) < std::numeric_limits<float>::epsilon() );
