...
double linear_aniso_distribution2::sample() 
{
    double samp = (-2.0*pow(Urand(),1.5))+1.0;
    return samp;
}
...
point forwardPeaked_distribution::sample() {
  // sample polar cosine with rejection sampling
  bool keep = false;
  double mu;
  double y;
  double eval_f;
  ...
  while (keep==false)
  {
    mu = 2*Urand() - 1;
    y  = Urand()*1.5;
    eval_f = (1.0/6.0)*( 1.0 + pow(( mu + 1.0 ),3.0) );
    //std::cout<<"y="<<y<<" eval_f="<<eval_f<<std::endl;
    //std::cout<<std::endl;
    
    if ( y < eval_f ){keep = true;}
  }
...
point shell_distribution::sample() {
  ...
  point p;
  p.x = rad*sin_mu*std::cos( azi );
  p.y = rad*sin_mu*std::sin( azi );
  p.z = rad*std::acos(mu);
...
point xbeam_distribution::sample() {
  // sample azimuthal angle uniformly, and radius with power law
  double azi = twopi * Urand();
  double r = pow(Urand(),0.5)*Rad;
  ...
  point p;
  p.x = x0;
  p.y = r*std::cos( azi )+y0;
  p.z = r*std::sin( azi )+z0;
...
double eDependentXS_distribution::sample(){};

double eDependentXS_distribution::sample2(particle* p) {
  double xs;
  xs = a + b*pow(p -> erg(),0.5);
  return xs;
}