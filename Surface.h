...
class cylinder : public surface{
  private:
    double x0, y0, z0, rad;
  public:
    cylinder( std::string label, double p1, double p2, double p3, double p4) :
      surface(label), x0(p1), y0(p2), z0(p3), rad(p4) {};
    ~cylinder(){};
    double eval( point p );
    double distance( ray r );
...
class cone_x : public surface{
  private:
    double x0, y0, z0, rad;
  public:
    cone_x( std::string label, double p1, double p2, double p3, double p4) :
      surface(label), x0(p1), y0(p2), z0(p3), rad(p4) {};
    ~cone_x(){};
    double eval( point p );
    double distance( ray r );
